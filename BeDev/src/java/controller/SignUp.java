...
public class SignUp extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (accountDAO.validEmail(email)) {
            int accountID = accountDAO.signup(new Student(new Account(0, email, password, true), name, 0, ""));
            if (accountID != 0) {
                String senderEmail = "";       // Email here
                ...
                boolean sendEmailSuccess = Email.sendMail(email, senderEmail, "Your email verification", message, emailPassword);
                if (sendEmailSuccess) {
